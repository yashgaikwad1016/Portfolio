      ...
      <div className="section-container">
        ...
        <div className="glass-card p-6 sm:p-10 space-y-10">
          ...
          <div className="border-b border-border/50 pb-6">
            <h2 className="text-2xl sm:text-3xl font-bold gradient-text">{personalInfo.name}</h2>
            <p className="text-muted-foreground mt-1">{personalInfo.role} • {personalInfo.location}</p>
            <p className="text-sm text-muted-foreground mt-2 max-w-2xl">{personalInfo.bio}</p>
          </div>
            ...
            <div className="space-y-6">
              {resumeData.experience.map((exp, i) => (
                ...
                >
                  ...
                  <p className="text-sm text-primary font-mono">{exp.company} • {exp.period}</p>
            ...
            <div className="space-y-4">
              {personalInfo.education.map((edu, i) => (
                <div key={i} className="pl-6 border-l-2 border-accent/30">
                  ...
                  <p className="text-sm text-accent font-mono">{edu.institution} • {edu.year}</p>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {skillCategories.flatMap((c) => c.skills).map((s) => (
                <span key={s.name} className="px-3 py-1 text-sm rounded-md bg-secondary text-secondary-foreground">
                  {s.name}
                </span>
              ))}
            </div>
